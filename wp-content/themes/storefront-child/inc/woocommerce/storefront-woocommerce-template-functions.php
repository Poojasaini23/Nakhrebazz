...
?>
    ...
    if (!function_exists('storefront_product_user')) {
        /**
         ...
         * @return void
         */
        function storefront_product_user()
        {
    ...
    if (!function_exists('storefront_product_search')) {
        ...
        ?>
            <div class="site-search">
                ...
                <span class="search-icon-bar desktop-search-bar">
                    <div class="search-toggle closed">
                        <button class="search-icon icon-search"><i class="fa fa-fw fa-search"></i></button>
                        <button class="search-icon icon-close"><i class="bi bi-x"></i></button>
                    </div>
    ...
    if (!function_exists('storefront_sorting_wrapper')) {
        ...
        function storefront_sorting_wrapper()
        {
            ...
            echo do_shortcode('[searchandfilter id="filter"]  [searchandfilter id="product_filter"]');
        }
    }
        ...
        function storefront_recent_products($args)
        {
            $args = apply_filters(
                ...
                array(
                    ...
                    'orderby' => 'date',
                    'order' => 'desc',
                    'title' => __('FEATURED CELEBRITY', 'storefront'),
                    'description' => __('SHOP  GET IT CUSTOMISED', 'storefront')
                )
            );
            ...
            if (false !== strpos($shortcode_content, 'product')) {
                ...
                echo '<h2 class="section-title celebrity-title">' . wp_kses_post($args['title']) . '</h2>';

                echo '<div class="product-celebrity-description">' . wp_kses_post($args['description']) . '</div>';
        ...
        function storefront_popular_products($args)
        {
            $args = apply_filters(
                ...
                array(
                    ...
                    'orderby' => 'rating',
                    'order' => 'desc',
                    'title' => __('DRESS FOR THE OCCASSION', 'storefront'),
                )
            );
            ...
            <div class="col-full">
                <div class="storefront-sticky-add-to-cart__content">
                    ...
                    <div class="storefront-sticky-add-to-cart__content-product-info">
                        <span class="storefront-sticky-add-to-cart__content-title"><?php esc_html_e('You\'re viewing:', 'storefront'); ?>
...
<?php
    ...
    if (!function_exists('storefront_related_products')) {
        /**
         * Display Recent Products
         * Hooked into the `homepage` action in the homepage template
         *
         ...
         * @return void
         */
        function storefront_related_products($args)
        {
            $args = apply_filters(
                'storefront_related_products_arg',
                array(
                    'limit' => 4,
                    'columns' => 4,
                    'orderby' => 'date',
                    'order' => 'desc',
                    'title' => __('RELATED PRODUCT', 'storefront'),
                )
            );
            ...
            $shortcode_content = storefront_do_shortcode(
                'products',
                apply_filters(
                    'storefront_related_products_shortcode_args',
                    array(
                        'orderby' => esc_attr($args['orderby']),
                        'order' => esc_attr($args['order']),
                        'per_page' => intval($args['limit']),
             ...
             */
            if (false !== strpos($shortcode_content, 'product')) {
                echo '<section class="storefront-product-section storefront-recent-products" aria-label="' . esc_attr__('Related Products', 'storefront') . '">';
    ...
    if (!function_exists('storefront_recently_products')) {
        /**
         * Display Recent Products
         * Hooked into the `homepage` action in the homepage template
         *
         * @since  1.0.0
         * @param array $arg the product section arg.
         * @return void
         */
        function storefront_recently_products($arg)
        {
            $arg = apply_filters(
                'storefront_recently_products_arg',
                array(
                    'limit' => 4,
                    'columns' => 4,
                    'orderby' => 'date',
                    'order' => 'desc',
                    'title' => __('RECENTLY VIEWED', 'storefront'),
                )
            );
            ...
            $shortcode_content = storefront_do_shortcode(
                'products',
                apply_filters(
                    'storefront_recently_products_shortcode_args',
                    array(
                        'orderby' => esc_attr($arg['orderby']),
                        'order' => esc_attr($arg['order']),
                        'per_page' => intval($arg['limit']),
                        'columns' => intval($arg['columns']),
                    )
                )
            );
             ...
             */
            if (false !== strpos($shortcode_content, 'product')) {
                echo '<section class="storefront-product-section storefront-recent-products" aria-label="' . esc_attr__('Recently Products', 'storefront') . '">';

                do_action('storefront_homepage_before_recent_products');

                echo '<h2 class="section-title">' . wp_kses_post($arg['title']) . '</h2>';
